...
{
	$statement = $connect->prepare("DELETE FROM parking_user WHERE user_id = ?");
...
$(document).ready(function() {
    $('#users-table').DataTable({
        ...
        "columns": [
            ...
            { "data": "user_updated_on"},
            {
        		"data": null,
        		"render": function(data, type, row) {
          			return '<a href="edit_user.php?id='+row.user_id+'" class="btn btn-sm btn-primary">Edit</a>&nbsp;<button type="button" class="btn btn-sm btn-danger delete_btn" data-id="'+row.user_id+'">Delete</button>';
        		}
        	}
        ]
    });
