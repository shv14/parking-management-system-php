...
function Currency_list()
{
	...
	foreach($data as $row)
	{
		$html .= '<option value="'.$row["code"].'" '.$action.'>'.$row["name"].'</option>';
	}
	return $html;
}
...
{
	$result = $connect->query('SELECT * FROM parking_category ORDER BY category_name ASC');

	$output = '';

	foreach($result as $row)
	{
		//$option_value = ($value == 'category_id') ? $row['category_id'] : $row['category_name'];
...
function duration_list($connect)
{
	$result = $connect->query('SELECT * FROM parking_duration ORDER BY duration_value ASC');

	$output = '';

	foreach($result as $row)
	{
		$output .= '<option value="'.$row["duration_id"].'">'.$row["duration_value"].' Hour</option>';
	}

	return $output;
}

function parking_slot_list($connect, $category_id)
{
	$result = $connect->query('SELECT * FROM parking_slot WHERE vehicle_category_id = "'.$category_id.'" AND parking_slot_status = "Available" ORDER BY parking_slot_name ASC');

	$output = '';

	foreach($result as $row)
	{
		$output .= '<option value="'.$row["parking_slot_id"].'">'.$row["parking_slot_name"].'</option>';
	}

	return $output;
}

/*function parking_duration_list($connect, $category_id, $duration_id)
{
	$result = $connect->query('SELECT * FROM parking_price WHERE category_id = "'.$category_id.'" AND duration_id = "'.$duration_id.'" ORDER BY price_id DESC');
